...
import { isSecureDownloadRecord, SecureDownloadRecord } from "../../types/secureDownloadRecord";
import { getEnv } from "../../utils/getEnv";
import { ddbClient } from "./dynamoDbClient";
export const getSecureDownloadRecord = async (downloadHash: string): Promise<SecureDownloadRecord | null> => {
  const params = {
  ...
  if (!isSecureDownloadRecord(record)) {
    throw new Error('Secure download data returned from db was not of correct type')
  }
